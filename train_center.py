...
from utils.center_util import _tranpose_and_gather_feature, load_model
from utils.center_image import transform_preds
from utils.center_losses import _neg_loss, _reg_loss
from utils.center_summary import create_summary, create_logger, create_saver, DisablePrint
from utils.center_post_process import ctdet_decode
from torchsummary import summary
...
def main():
    ...
    '''
    ckpt = torch.load('/home/wangchunyu/PyTorch-Spiking-YOLOv3-ultralytics/results_route/2021-02-10_15-04-03/checkpoint.pth.tar',map_location=cfg.device)
    # load model
    try:
        print('load*******************')
        ...
        #    k_new=k.split('.',1)[1]
        #    print(k_new)
        ckpt['state_dict'] = {k.split('.',1)[1]: v for k, v in ckpt['state_dict'].items() if k.split('.',1)[1] in model.state_dict().keys() and model.state_dict()[k.split('.',1)[1]].numel() == v.numel()}
    ...
    for epoch in range(1, cfg.num_epochs + 1):
        ...
        if cfg.val_interval > 0 and epoch % cfg.val_interval == 0:
            val_map(epoch)
        print(saver.save(ema.ema.module.state_dict() if hasattr(model, 'module') else ema.ema.state_dict(), 'checkpoint'))
