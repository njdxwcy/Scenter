...
from utils.center_image import get_border, get_affine_transform, affine_transform, color_aug
from utils.center_image import draw_umich_gaussian, gaussian_radius
...
class COCO(data.Dataset):
    def __init__(self, data_dir, split, split_ratio=1.0, img_size=512):
        ...
        self.img_dir = os.path.join(self.data_dir,'images', '%s2014' % split)
        if split == 'test':
            self.annot_path = os.path.join(self.data_dir, 'annotations', 'instances_val2014.json')
        else:
            self.annot_path = os.path.join(self.data_dir, 'annotations', 'instances_%s2014.json' % split)
